...
suite("fills at the newest block", () => {
  ...
  const row: TenantRow = { id, bot_id: botId, bot_username: "test", bot_token_sealed: sealer.seal("test"), aomi_webhook_url: "http://test.invalid", webhook_secret: "s", ingest_key_hash: "h", ingest_origins: [] };
  ...
  beforeAll(async () => {
    ...
    await store.recordPendingHandover({ tenant: id, tokenHash: "test", accountId: "21", chainId: 1, ownerAddress: "0xowner" });
    await store.bindStart(id, "test", "42");
