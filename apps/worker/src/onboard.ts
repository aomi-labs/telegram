...
import { BotApi, Sealer, Store, migrate, randomToken } from "@aomi-telegram/core";
import { loadConfig } from "./config.ts";
import { tenants } from "./tenants.ts";

const { values } = parseArgs({ options: { tenant: { type: "string" }, "aomi-webhook-url": { type: "string" } } });
...
console.log(JSON.stringify({ tenant: tenantId, bot: me.username, bot_id: me.id, webhook: ourUrl }, null, 2));
await sql.end();
